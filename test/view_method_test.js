...
exports["view doument operations"] = {
  ...
  setUp: function (done) {
    ...
    fixture = this.fixture = {
      views: {'by_model': {map: "function (doc) { emit(doc.class_name, 1); }"}}
    };
    ...
    function setUpdatePath() {
      return {path: '/'+ dbname +'/_design/'+ updateDoc, data: fixture};
    }
  ...
  "PUT/update and GET": function (test) {
    ...
    function putDocument() {
      var doc = fixture;
      doc.views.by_model.reduce = "function (keys, values) { return sum(values); }";
    ...
    function getView(responses) {
      var promise = COUCH.request({
        method: 'GET'
      , path: '/'+ dbname +'/_design/'+ docname + '/_view/by_model'
      , hostname: HOSTNAME
      , port: PORT
      , username: USERNAME
      , password: PASSWORD
    ...
    function runTests(responses) {
      ...
      test.equal(typeof get.body._rev, 'string', 'GET doc._rev')
      test.equal(put.body.rev, get.body._rev, 'correct revision')
      test.equal(get.body.views.by_model.map, 'function (doc) { emit(doc.class_name, 1); }', 'doc.views map')
      test.equal(get.body.views.by_model.reduce, 'function (keys, values) { return sum(values); }', 'doc.views reduce')

      // Test GET view
      test.strictEqual(getView.statusCode, 200, 'GET status code')
      test.assertJSON(getView, 'GET JSON')
      test.ok(Array.isArray(getView.body.rows), 'GET view rows')
      test.ok(typeof getView.body.rows[0].value === 'number', 'GET view value')
